...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// Triangulos preenchidos com amarelo
...
int createCircle(int nPoints, float radius){
	...
	float slice = 2 * Pi / (float)nPoints;//o quanto que aumenta do angulo a cada ciclo (uso radianos)
	float angle = 0.0 + 2*slice;
	...
	{
		float x = radius * cos(angle); //(Xc + radius) * cos(angle) -> se nao fosse 0.0
